---
name: new-page
description: Add a new locale-routed page under app/[locale]/ in this Next.js app-router repo. Use when the user asks to add a page, add a route, or create a new section of the site. Ensures generateStaticParams, the next/root-params locale pattern, and matching message keys land in all three locale files in one pass, since a missing key in one locale fails silently only when that locale is visited.
---
...
1. Create `app/[locale]/<route>/page.tsx` in this shape. `generateStaticParams()` is required on every page. Don't take a `params` prop just for the locale and don't call `setRequestLocale` (deprecated): next-intl reads the locale from `next/root-params` in `i18n/request.ts`, which also validates it (`hasLocale`/`notFound()`) — don't repeat that per-page. When the page needs the locale value itself (metadata, JSON-LD, a `redirect`), use `await getLocale()`.
